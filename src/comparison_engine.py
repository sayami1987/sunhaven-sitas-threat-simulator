...
def compare_scenario(env, scenario, risk_model, controls, selected_ids, *,
    ...
    for key, path in before_by_edges.items():
        ...
        finding = {"findingId": finding_id, "pathId": path.id, "scenarioId": scenario.id,
                   ...
                   "edgeDetails": [{"edgeId": edge_id, "source": graph.edges[edge_id].source,
                                    "target": graph.edges[edge_id].target,
                                    "relationship": graph.edges[edge_id].relationship,
                                    "explanation": graph.edges[edge_id].explanation,
                                    "baselineLikelihood": graph.edges[edge_id].likelihood,
                                    "controlledLikelihood": applied.graph.edges[edge_id].likelihood
                                    if edge_id in applied.graph.edges else None} for edge_id in key],
